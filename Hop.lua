...
richText.ColorShortcuts.Green = Color3.new(0.333333, 1, 0.498039)
...
richText.ColorShortcuts.Grass = Color3.new(0.333333, 0.666667, 0)
richText.ColorShortcuts.Dirt = Color3.new(0.415686, 0.223529, 0.0352941)
richText.ColorShortcuts.Stone = Color3.new(0.623529, 0.631373, 0.67451)
richText.ColorShortcuts.Rock = Color3.new(0.415686, 0.423529, 0.45098)
richText.ColorShortcuts.Obsidian = Color3.new(0.384314, 0.145098, 0.819608)
richText.ColorShortcuts.Lava = Color3.new(0.854902, 0.521569, 0.254902)
richText.ColorShortcuts.Coal = Color3.new(0.14902, 0.231373, 0.294118)
richText.ColorShortcuts.Copper = Color3.new(0.627451, 0.372549, 0.207843)
richText.ColorShortcuts.Iron = Color3.new(0.576471, 0.576471, 0.576471)
richText.ColorShortcuts.Gold = Color3.new(0.960784, 0.803922, 0.188235)
richText.ColorShortcuts.Diamond = Color3.new(0.0352941, 0.537255, 0.811765)
richText.ColorShortcuts.Emerald = Color3.new(0, 1, 0)
richText.ColorShortcuts.Chest = Color3.new(1, 1, 0.498039)
...
defaults.TextStrokeTransparency = 0.5
...
function animationStyles.Fade(character, animateAlpha, properties)
	...
	if character:IsA("TextLabel") then
		...
		character.TextStrokeTransparency = 1 - (animateAlpha * (1 - properties.TextStrokeTransparency))
...
NotificationTemplate.AutomaticSize = 'Y'
NotificationTemplate.BackgroundTransparency = 1
NotificationTemplate.Position = UDim2.fromOffset(-300, 0)
NotificationTemplate.Size = UDim2.fromOffset(750, 0)
NotificationTemplate.TextWrapped = true

local Template = {
	DEFAULT_MESSAGE = "<EMPTY MESSAGE>", 
	DEFAULT_DURATION = 6,
	START_POSITION = UDim2.new(0.5, 0, 0, 10), 
	Y_PADDING = 27, 
	LABEL_TEMPLATE = NotificationTemplate, 
	PARENT_GUI = Gui, 
...
Template.PARENT_GUI.Name = "Notifications"
local MessagesCache = {
	Messages = {}, 
	Side = 0
}
local TweenService = game:GetService("TweenService")
function MessagesCache.Add(Cache, Properties)
	local Position = Template.START_POSITION - UDim2.new(0, 0, 0, Template.START_POSITION.Y.Offset + Template.Y_PADDING * 1.5)
	if not Properties.FirstTime then
		Properties.FirstTime = true
		Properties.Label.Position = Position
	end

	local Index = 0

	for i, v in pairs(Cache.Messages) do
		Index += v.Label.AbsoluteSize.Y
	end

	Properties.In = TweenService:Create(Properties.Label, TweenInfo.new(0.3), {
		Position = Template.START_POSITION + UDim2.new(0, 0, 0, Index)
	});
	Properties.In:Play()
	table.insert(Cache.Messages, Properties)
end;
function MessagesCache.Update(Cache)
	for Properties, Notification in next, Cache.Messages do
		if Notification:Dead() then
			table.remove(Cache.Messages, Properties)
			local Side
			if Cache.Side == 1 then
				Side = -1
			else
				Side = 1
			end;
			Cache.Side = Side
			Notification.Out = TweenService:Create(Notification.Label, TweenInfo.new(0.5), {
				Position = Notification.Label.Position + UDim2.new(0.5 * Cache.Side, 300 * Cache.Side, 0, 0)
			})
			Notification.Out.Completed:Connect(function()
				Notification.Label:Destroy()
			end)
			Notification.Out:Play()
			local Cache2 = {}
			for _, Properties in next, Cache.Messages do
				table.insert(Cache2, Properties)
			end
			Cache.Messages = {}
			for _, Properties in next, Cache2 do
...
function Notification.new(MSG, Duration, Name)
	MSG = MSG or Template.DEFAULT_MESSAGE
	Duration = Duration or Template.DEFAULT_DURATION
	local Text = Template.LABEL_TEMPLATE:Clone()
	for Index, Value in pairs(Template.NEW_PROPERTIES) do
		Text[Index] = Value
	end
	Name = Name or Text.Name
	Text.Parent = Template.PARENT_GUI

	local textPos = 1
	local textLength = #MSG
	local labelSeries = {}
	...
	local function printSeries(labelSeries)	
		for _, t in pairs(labelSeries) do
			local markupKey, markupValue = string.match(t, "<(.+)=(.+)>")
			if markupKey and markupValue then	
			else
	...
	while textPos and textPos <= textLength do
		local nextMarkupStart, nextMarkupEnd = string.find(MSG, "<.->", textPos)
		local nextSpaceStart, nextSpaceEnd = string.find(MSG, "[ \t\n]", textPos)
		...
		local nextWord = nextBreakStart > textPos and string.sub(MSG, textPos, nextBreakStart - 1) or nil
		local nextBreak = nextBreakStart <= textLength and string.sub(MSG, nextBreakStart, nextBreakEnd) or nil
		table.insert(labelSeries, nextWord)
	...
	local TextObject = richText:New(Text, MSG)
	TextObject:Animate()
...
function Notification.Dead(Notify)
	return Notify.Duration < tick() - Notify.CreationTime
end
function Notification.Display(Notify)
	if Notify.Displayed then
		return false
	end
	Notify.Displayed = true
	Notify.CreationTime = tick()
	Notify.Label.Visible = true
	MessagesCache:Add(Notify)
	return true
end
function Notification.Remove(Notify)
	Notify.Duration = 0
end
